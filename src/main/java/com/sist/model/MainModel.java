 ...
 * 		JSP
 * 			1) 자바 기반의 서버 사이드 웹 기술
 * 			2) HTML 내부에 자바 코드를 첨부해서 동적페이지 생성
 ...
 *							= errorPage : 에러 발생시 이동하는 파일을 설정
 *							  errorPage="error.jsp"
 *								=> 기본 : forward를 사용하고 있다
 *								*** URL 주소 (request를 가지고 있다)
 *							= a.jsp => b.jsp
 *									| request가 초기화
 *							= a.jsp
 *								=> b.jsp 포함
 ...
 *					setHeader() => 다운로드
 *			session => HttpSession
 *				= 상태 유지 (데이터 유지 => JSP 전체에서 공유)
 *				= 서버에 저장
 *				= 저장 : setAttribute()
 *				= 읽기 : getAttribute()
 *				= 삭제 : removeAttribute() => 일부 삭제 (장바구니)
 ...
 *				=> ${requestScope.no }
 *				=> ${no } => requestScope 생략 가능
 *			${sessionScope.키}
 *			=> 연산자 : 숫자 / 문자 / 날짜
 *			   산술 : + , - , *, / , %
 *					1) null 값이 있는 경우 => null + 1 => 1 (null은 0으로 취급)
 *					2) +는 순수하게 더하기 기능만 있다
 *						** 문자열 결합 +=
 *					3) / 는 정수 / 정수 => 실수
 *				비교 연산자 : ==, !=, <, >, <=, >=
